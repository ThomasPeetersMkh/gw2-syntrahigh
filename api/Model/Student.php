...
class Student {
  ...
  private function checkEmail() {
    ...
    $sqlQuery = "SELECT llg_email FROM " . $this->db_table . " WHERE llg_email like '" . $email[0] . '%' . $email[1] . "' order by llg_email asc limit 1";
    $stmt = $this->conn->prepare($sqlQuery);
    $stmt->execute();
    $dataRow = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($dataRow['llg_email']) {
      $email = explode("@", $dataRow['llg_email']);
      $lastChar = substr($email[0], -1);
      if (preg_match("/^\d+$/", $lastChar) === 1) {
        $numb = intval($lastChar);
        $numb++;
        $this->email = str_replace($lastChar, strval($numb), $email[0]) . "@" . $email[1];
      }
      else {
        $this->email = $email[0] . "1@" . $email[1];
      }
    }
  }
